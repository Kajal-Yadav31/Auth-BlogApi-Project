...
from BlogApi.serializers import BlogSerializer, CategorySerializer, BlogCommentSerializer
...
class BlogListView(generics.ListAPIView):
    ...
    # Custom Throttle
    throttle_classes = [BlogListViewThrottle]

    # Filtering
    filter_backends = [DjangoFilterBackend, filters.SearchFilter]
    filterset_fields = ['category__category_name', 'is_public']
    search_fields = ['^blog_title',
                     'blog_description', 'category__category_name']
...
class BlogCreateView(generics.CreateAPIView):
    ...
    serializer_class = BlogSerializer
    permission_classes = [IsAuthenticatedOrReadOnly]
    throttle_classes = [BlogCreateViewThrottle]


# to automatically select the login user as author field
...
class BlogCommentDetailView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def get_object(self):
        ...
        if comment.blog.id != blog_id:
            raise serializers.ValidationError(
                {"Message": "This comment is not related to the requested blog"}, status=status.HTTP_401_UNAUTHORIZED)
        return comment
